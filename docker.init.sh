...
NV_GPU=$i nvidia-docker run -it -p 0.0.0.0:9767:22/tcp -p 0.0.0.0:9769:9769/tcp --name torchy  torch-server:$TORCH_SERVER_V --runtime=nvidia --ipc=host -e NVIDIA_VISIBLE_DEVICES=$i -e NVIDIA_DRIVER_CAPABILITIES=compute,utility -v $d:/opt/entrypoint/dataset
