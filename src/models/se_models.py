...
class SEModule(nn.Module):
    ...
    def __init__(self, channels, reduction, channels_out=None):
        super(SEModule, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc1 = nn.Conv2d(
            channels, 
            channels // reduction if channels_out is None else channels_out, 
        ...
        self.fc2 = nn.Conv2d(
            channels // reduction if channels_out is None else channels_out,
            channels if channels_out is None else channels_out, 
            kernel_size=1, padding=0
        )
    ...
    def forward(self, x):
        ...
        if self.channels_out is not None:
            module_input = self.fc1(module_input)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.se_module(out_) + residual
        out = self.relu(out)

        if self.integrate:
            integrated_out = self.se_module_integrate(out_ + residual)
            return out, self.relu(integrated_out)
        return out
...
class SEResNeXtBottleneck(Bottleneck):
    ...
    def __init__(
        ...
        if integrate:
            self.se_module_integrate = SEModule(
                planes * 4, reduction=reduction, channels_out=planes)
        self.downsample = downsample
        self.stride = stride
...
class SENet(nn.Module):
    ...
    def _make_layer(
        ...
        for i in range(1, blocks):
            layers.append(block(
                self.inplanes, planes, groups, reduction, integrate=i==blocks-1))
...
def init_pretrained_weights(model, model_url):
    ...
    try:
        ...
        path = str(config.PATHS.MODELS/os.path.basename(model_url))
